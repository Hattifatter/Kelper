...
from processing_pandas import process_pandas_dataset
...
def run() -> None:
    ...
    print("=== PANDAS PAIR EXPERIMENTS WITH DASH ===")
    ...
    for dataset_name, input_file in DATASETS.items():
        ...
        print(
            f"Pandas processed: rows_in={processed_meta['rows_input']}, "
        ...
        for visualization_method, visualizer in VISUALIZERS.items():
            print(f"Building visualization: pandas + {visualization_method}")
            ...
            row = {
                ...
                "pair": f"pandas + {visualization_method}",
                "processing_time_sec": processing_metrics.elapsed_sec,
                "processing_memory_peak_mb": processing_metrics.memory_peak_mb,
    ...
    metrics_file = METRICS_DIR / "pandas_pair_metrics_with_dash.csv"
    metrics.to_csv(metrics_file, index=False, encoding="utf-8-sig")

    # Also overwrite the generic Pandas metrics file so later notebooks can read one latest table.
    latest_metrics_file = METRICS_DIR / "pandas_pair_metrics_latest.csv"
    ...
    print(f"Latest metrics: {latest_metrics_file}")
